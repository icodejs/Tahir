...
function fragmentBuilder(fc, callback) {
	...
	var fragmentCount = _.keys(fc.fragments).length;

	for (var _key in fc.fragments) {

		if (fc.fragments.hasOwnProperty(_key)) {

			var frag = fc.fragments[_key];

			getFragment({ root: fc.root, map: fc.map, item: frag, key: _key }, function (err, key, htmlTemplate) {
...
function mastheadMapper(fragment, callback) {
	...
	jsdom.jQueryify(window, jquery, function() {
		...
		var updated_socialLinks = plates.bind($socialLinks.html(), fragment.item.data, fragment.map);
